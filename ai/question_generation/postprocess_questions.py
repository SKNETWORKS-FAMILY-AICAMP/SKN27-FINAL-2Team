...
def import_db(args: argparse.Namespace) -> int:
    questions = load_questions(args.input)
    ml_warning_count = sum("ML주의" in (question.get("review_tags") or []) for question in questions)
    ...
    replacement_sql = """
        UPDATE questions
        SET q_score = %s, era = %s, topic = %s, question_type = %s,
            question_subtype = %s, content = %s, passage = %s,
            image_caption = %s, question_image_path = %s, answer_no = %s,
            answer_explanation = %s, core_concept = %s
        WHERE source_key = %s
        RETURNING question_id
    """
    option_sql = """
    ...
    """
    replacement_option_sql = """
        UPDATE question_options
        SET content = %s, choice_image_path = %s, is_answer = %s, choice_explanation = %s
        WHERE question_id = %s AND choice_no = %s
    """
            ...
            for question, (question_row, option_rows) in zip(questions, prepared):
                ...
                if key in existing:
                    if args.replace_existing:
                        cursor.execute(replacement_sql, (*question_row[2:], key))
                        row = cursor.fetchone()
                        if cursor.rowcount != 1 or not row:
                            raise RuntimeError(f"{key} 문항 UPDATE 대상이 정확히 1개가 아닙니다.")
                        question_id = row[0]
                        for option in option_rows:
                            cursor.execute(replacement_option_sql, (*option[1:], question_id, option[0]))
                            if cursor.rowcount != 1:
                                raise RuntimeError(
                                    f"{key}의 {option[0]}번 선지 UPDATE 대상이 정확히 1개가 아닙니다."
                                )
                        replaced += 1
                        continue
