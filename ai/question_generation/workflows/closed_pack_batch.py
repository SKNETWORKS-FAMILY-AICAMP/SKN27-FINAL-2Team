...
from ai.question_generation.core.exam_distribution import ERA_ORDER, apportion, official_distribution
from ai.question_generation.evaluation.v18 import DEFAULT_MIN_ACCEPT_SCORE
from ai.question_generation.generation.material_rules import DEFAULT_MATERIAL_PROMPT_RULES, load_json_dict
from ai.question_generation.retrieval.closed_pack_input import plan_variants
from ai.question_generation.workflows.question_pipeline import invalidate, main as run_question


LABELS = "①②③④⑤"
ML_WARNING_TAG = "ML주의"
CHOICE_QUALITY_ROOT = Path(__file__).resolve().parents[2] / "ml_question" / "choice_quality_model"
CHOICE_QUALITY_SCRIPT = CHOICE_QUALITY_ROOT / "predict_choice_quality.py"
CHOICE_QUALITY_MODEL = CHOICE_QUALITY_ROOT / "model"
...
def run_choice_quality_review(
    output_dir: Path,
    questions: list[dict[str, Any]],
    *,
    script: Path = CHOICE_QUALITY_SCRIPT,
    model_dir: Path = CHOICE_QUALITY_MODEL,
) -> dict[str, Any]:
    """최종 문항의 텍스트 선지를 v15 모델로 검수하고 결과만 저장한다."""
    eligible = [question for question in questions if question.get("choice_mode") != "image"]
    excluded = len(questions) - len(eligible)
    if not eligible:
        tagged_path = output_dir / "evaluation" / "choice_quality_tagged_questions.json"
        write_json_atomic(tagged_path, {"count": len(questions), "questions": questions})
        return {
            "status": "SKIPPED",
            "reason": "no_text_choice_questions",
            "excluded_image_questions": excluded,
            "tagged_questions": str(tagged_path),
        }
    required = [script, model_dir / "config.json", model_dir / "model.safetensors", model_dir / "tokenizer.json"]
    missing = [str(path) for path in required if not path.is_file()]
    if missing:
        raise RuntimeError(f"choice quality model files are missing: {missing}")

    evaluation_dir = output_dir / "evaluation"
    input_path = evaluation_dir / "choice_quality_input.json"
    csv_path = evaluation_dir / "choice_quality_review.csv"
    json_path = evaluation_dir / "choice_quality_review.json"
    write_json_atomic(input_path, eligible)
    ...
    if completed.returncode:
        ...
        raise RuntimeError(f"choice quality review failed ({completed.returncode}): {detail[-2000:]}")
    report = read_json(json_path)
    model_summary = report.get("summary") or {}
    rows = report.get("rows")
    if not isinstance(rows, list):
        raise RuntimeError("choice quality review output rows are missing")
    expected_choices = sum(len(question.get("choices") or []) for question in eligible)
    if int(model_summary.get("total_choices") or -1) != expected_choices:
        raise RuntimeError("choice quality review output count does not match its input")
    warning_ids = {
        str(row.get("문항ID") or "")
        for row in rows
        if isinstance(row, dict) and row.get("검수상태") == "검수필요" and row.get("문항ID")
    }
    for question in questions:
        question_id = str(question.get("question_id") or question.get("id") or question.get("seed_id") or "")
        if question_id in warning_ids:
            tags = list(question.get("review_tags") or [])
            if ML_WARNING_TAG not in tags:
                tags.append(ML_WARNING_TAG)
            question["review_tags"] = tags
    tagged_path = evaluation_dir / "choice_quality_tagged_questions.json"
    write_json_atomic(tagged_path, {"count": len(questions), "questions": questions})
    return {
        "status": "COMPLETE",
        "profile": "choice_quality_v15_advisory",
        "questions": len(eligible),
        "excluded_image_questions": excluded,
        "warning_question_count": len(warning_ids),
        "warning_question_ids": sorted(warning_ids),
        **model_summary,
        "csv": str(csv_path),
        "json": str(json_path),
        "tagged_questions": str(tagged_path),
...
def evaluate_questions(args: argparse.Namespace, questions: list[dict[str, Any]], cycle: int) -> list[dict[str, Any]]:
    ...
    command = [
        sys.executable, "-m", "ai.question_generation.evaluation.v18",
        "--input", str(input_path),
...
def main() -> int:
    ...
    if (
        ...
        and questions
        and (summary.get("evaluation") or {}).get("status") in {"PASS", "FAIL"}
    ):
        try:
            summary["choice_quality_review"] = run_choice_quality_review(args.output_dir, questions)
        except (OSError, RuntimeError, ValueError, KeyError) as exc:
            summary["choice_quality_review"] = {"status": "ERROR", "error": str(exc)}
