...
def length_bias_choice_numbers(q: dict[str, Any]) -> set[int]:
    ...
    # 너무 민감하면 정상 기출도 잡을 수 있으므로, 최소 차이와 비율을 함께 본다.
    choices = q.get("choices") or []
    answers = answer_choice_numbers(q)
    lengths = [len(choice_text(choice).strip()) for choice in choices]
    positive_lengths = [length for length in lengths if length > 0]
    if len(positive_lengths) < 5:
        return set()

    sorted_lengths = sorted(positive_lengths)
    ...
    for idx, choice in enumerate(choices, start=1):
        ...
        too_short = length <= 8 or (length <= median_length * 0.45 and median_length - length >= 10)
        too_long = length >= median_length * 2.2 and length - median_length >= 25
...
def generated_question_rows(q: dict[str, Any], audit: dict[str, Any]) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    question_id = f"generated_v8_{safe_id(seed_id)}_{source_hash}_{fp[:8]}"
    answer_no = answer_number(q)
...
def main() -> None:
    ...
    write_json(OUT_DIR / "choice_quality_data_v8.json", all_rows)
    write_json(OUT_DIR / "choice_quality_train_v8.json", splits["train"])
    write_json(OUT_DIR / "choice_quality_test_v8.json", splits["test"])
    write_json(OUT_DIR / "choice_quality_skipped_v8.json", skipped_items)
    write_json(OUT_DIR / "choice_quality_rule_only_v8.json", rule_only_items)

    error_codes = sorted({code for row in all_rows for code in row.get("error_codes", [])})
    summary = {
        "version": "v8_choice_level_refined_with_rule_context",
        ...
        "additional_note": "추가분류 511문항 파일은 v8 BERT 학습에는 직접 사용하지 않는다. 대부분 문항/메타데이터 단위 사유이기 때문이다.",
        ...
        "files": [
            ...
            "choice_quality_data_v8.json",
            "choice_quality_skipped_v8.json",
            "choice_quality_rule_only_v8.json",
        ],
    }
    write_json(OUT_DIR / "choice_quality_summary_v8.json", summary)
