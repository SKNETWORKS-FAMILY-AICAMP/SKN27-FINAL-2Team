...
# v14 선지 단위 이진 분류 학습 노트북 생성기입니다.
OUT = Path(__file__).resolve().parent / "train_choice_quality_runpod_v14.ipynb"
...
v14는 문항 전체 사용불가 여부가 아니라 **선지 자체의 오류 유무**만 학습합니다.  
...
```text
/workspace/
├─ train_choice_quality_runpod_v14.ipynb
└─ common/
...
# 운영/비교 기준을 흔들지 않기 위해 v14에서는 threshold를 0.1로 고정합니다.
...
OUTPUT_DIR = WORKSPACE_DIR / "choice_quality_output_v14"
...
def write_remaining_weird_file(path: Path, rows: list[dict[str, Any]], labels: np.ndarray, error_probs: np.ndarray) -> None:
    # v14 보강 후에도 WEIRD_CHOICE로 남은 케이스만 저장합니다.
    records = [
        record
