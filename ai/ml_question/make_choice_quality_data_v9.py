...
def generated_question_rows(q: dict[str, Any], audit: dict[str, Any]) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    question_id = f"generated_v9_{safe_id(seed_id)}_{source_hash}_{fp[:8]}"
    answer_no = answer_number(q)
...
def main() -> None:
    ...
    write_json(OUT_DIR / "choice_quality_data_v9.json", all_rows)
    write_json(OUT_DIR / "choice_quality_train_v9.json", splits["train"])
    write_json(OUT_DIR / "choice_quality_test_v9.json", splits["test"])
    write_json(OUT_DIR / "choice_quality_skipped_v9.json", skipped_items)
    write_json(OUT_DIR / "choice_quality_rule_only_v9.json", rule_only_items)

    error_codes = sorted({code for row in all_rows for code in row.get("error_codes", [])})
    summary = {
        "version": "v9_choice_level_refined_with_rule_context",
        ...
        "additional_note": "추가분류 511문항 파일은 v9 BERT 학습에는 직접 사용하지 않는다. 대부분 문항/메타데이터 단위 사유이기 때문이다.",
        ...
        "files": [
            ...
            "choice_quality_data_v9.json",
            "choice_quality_skipped_v9.json",
            "choice_quality_rule_only_v9.json",
        ],
    }
    write_json(OUT_DIR / "choice_quality_summary_v9.json", summary)
