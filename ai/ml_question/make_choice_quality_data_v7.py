...
def collect_rule_only(q: dict[str, Any], categories: list[str], reasons: dict[str, Any]) -> dict[str, Any] | None:
    ...
    duplicate_numbers = duplicated_choice_numbers(q)
...
def generated_question_rows(q: dict[str, Any], audit: dict[str, Any]) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    # API 카테고리에 없더라도 정답 선지가 극단적으로 짧으면 선지 단위 오류 후보로 추가한다.
    short_answers = short_answer_choice_numbers(q)
    if short_answers:
        add_error(abnormal_by_choice, short_answers, "ANSWER_LENGTH_BIAS")
    ...
    question_id = f"generated_v7_{safe_id(seed_id)}_{source_hash}_{fp[:8]}"
    answer_no = answer_number(q)
...
def main() -> None:
    ...
    write_json(OUT_DIR / "choice_quality_data_v7.json", all_rows)
    write_json(OUT_DIR / "choice_quality_train_v7.json", splits["train"])
    write_json(OUT_DIR / "choice_quality_test_v7.json", splits["test"])
    write_json(OUT_DIR / "choice_quality_skipped_v7.json", skipped_items)
    write_json(OUT_DIR / "choice_quality_rule_only_v7.json", rule_only_items)

    error_codes = sorted({code for row in all_rows for code in row.get("error_codes", [])})
    summary = {
        "version": "v7_choice_level_refined",
        "purpose": "선지 1개 단위로 오류 유무를 분류한다. BERT가 판단 가능한 선지 자체 오류만 학습하고, 중복/복수정답은 규칙/후처리로 분리한다.",
        ...
        "additional_note": "추가분류 511문항 파일은 v7 BERT 학습에는 직접 사용하지 않는다. 대부분 문항/메타데이터 단위 사유이기 때문이다.",
        ...
        "files": [
            ...
            "choice_quality_data_v7.json",
            "choice_quality_skipped_v7.json",
            "choice_quality_rule_only_v7.json",
        ],
    }
    write_json(OUT_DIR / "choice_quality_summary_v7.json", summary)
