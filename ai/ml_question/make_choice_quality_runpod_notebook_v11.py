...
# v11 선지 단위 이진 분류 학습 노트북 생성기입니다.
OUT = Path(__file__).resolve().parent / "train_choice_quality_runpod_v11.ipynb"
...
v11은 문항 전체 사용불가 여부가 아니라 **선지 자체의 오류 유무**만 학습합니다.  
중복 선지/복수 정답처럼 선지 5개 비교가 필요한 오류는 BERT 학습과 분리하고, 결과 저장 단계에서 규칙 검사로 함께 확인합니다.

v11 변경점:

- `ANSWER_LENGTH_BIAS`에서 `8글자 이하` 같은 절대 기준을 제거합니다.
- 정답 선지를 제외한 다른 선지들의 평균/중앙값과 비교해 유독 짧거나 긴 경우만 길이 편향으로 봅니다.
- 결과 CSV에 길이 판단 근거 컬럼을 저장합니다.

RunPod 폴더 구조:

```text
/workspace/
├─ train_choice_quality_runpod_v11.ipynb
└─ common/
...
OUTPUT_DIR = WORKSPACE_DIR / "choice_quality_output_v11"
...
def has_answer_length_bias(row: dict[str, Any]) -> bool:
    ...
    if avg_length <= 0 or median_length <= 0:
        return False
    too_short = text_length <= median_length * 0.45 and avg_length - text_length >= 10
    too_long = text_length >= median_length * 2.2 and text_length - avg_length >= 25
