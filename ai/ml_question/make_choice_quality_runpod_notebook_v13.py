...
# v13 선지 단위 이진 분류 학습 노트북 생성기입니다.
OUT = Path(__file__).resolve().parent / "train_choice_quality_runpod_v13.ipynb"
...
v13은 문항 전체 사용불가 여부가 아니라 **선지 자체의 오류 유무**만 학습합니다.  
...
- `(가)-(나)-(다)` 같은 순서형/기호형 선지는 `ORDER_CHOICE_CONTEXT_REQUIRED`로 분리해 문항 단위 확인 대상으로 둡니다.
...
```text
/workspace/
├─ train_choice_quality_runpod_v13.ipynb
└─ common/
...
# 운영/비교 기준을 흔들지 않기 위해 v13에서는 threshold를 0.1로 고정합니다.
...
OUTPUT_DIR = WORKSPACE_DIR / "choice_quality_output_v13"
...
def build_final_metrics(rows: list[dict[str, Any]], labels: np.ndarray, error_probs: np.ndarray) -> dict[str, Any]:
    ...
    return {
        ...
        "known_holes": [
            ...
            "ORDER_CHOICE_CONTEXT_REQUIRED는 선지 하나만으로 확정하지 말고 문항 전체를 확인해야 한다.",
...
def write_remaining_weird_file(path: Path, rows: list[dict[str, Any]], labels: np.ndarray, error_probs: np.ndarray) -> None:
    # v13 보강 후에도 WEIRD_CHOICE로 남은 케이스만 저장합니다.
    records = [
        record
