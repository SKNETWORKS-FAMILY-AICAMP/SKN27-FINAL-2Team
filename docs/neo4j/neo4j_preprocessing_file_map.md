...
| `make_term_person_review.py` | `neo4j/scripts/` | 동명이인 Term-Person 후보를 펼쳐 수동 검수 CSV 생성. runner 미포함, 수동 실행 |
...
| `term_person_review_approved.csv` | 사람이 승인한 Term-Person 수동 연결 목록. `TERM_PERSON` 결과만 기록하며 `REFERS_TO` 관계에 `MANUAL`로 반영 |
