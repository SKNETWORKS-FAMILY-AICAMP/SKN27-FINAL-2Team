...
def get_history_terms(problems: list[dict]) -> list[dict]:
    ...
    prompt = """다음 한국사 시험 문항들에서 역사적으로 고유한 의미를 가진 용어만 추출하라

[규칙]
- 입력 원문에 실제로 등장한 표현만 추출한다
- 새로운 용어를 추론하거나 생성하지 않는다.
- 인물, 사건, 국가, 왕조, 제도, 정책, 단체, 기관, 문헌, 문화재, 조약, 사상, 역사적 지명, 유물, 유적 등을 추출한다.
- '설명', '내용', '사회', '정부', '사건'같은 일반 명사는 제외한다
- (가), (나), ㉠, ㄱ 같은 자리 표시 기호는 용어가 아니다. 절대 추출하지 않는다.
- 조사와 수식어는 제거하고 고유한 명사구만 남긴다
...
[추출 예시]
아래는 문항 하나에 대한 추출 기준 예시다.

입력 문항:
단양 수양개 유적에서 출토된 이 슴베찌르개는 주먹도끼와 함께 (가) 시대의 대표적인 유물 중 하나이다. 이 유적에서는 슴베찌르개와 함께 돌날과 몸돌 등의 뗀석기 출토되었다.
(가) 시대의 사회 모습으로 옳은 것은?
주로 동굴이나 막집에 거주하였다.
가락바퀴를 이용하여 실을 뽑았다.
명도전을 이용하여 중국과 교역하였다.
...
추출해야 하는 용어:
단양 수양개 유적, 슴베찌르개, 주먹도끼, 돌날, 몸돌, 뗀석기, 막집, 가락바퀴, 명도전, 철제 농기구, 의례 도구, 청동 방울

제외해야 하는 표현:
'(가) 시대', '사회 모습', '유물', '유적', '교역', '농사', '중국' 같은 일반 표현
...
[
    {
        ...
        "terms": [
            {
                "raw_term": "슴베찌르개",
                "canonical_term": "슴베찌르개",
                "category": "유물"
            },
            {
                "raw_term": "독립 협회",
                "canonical_term": "독립 협회",
                "category": "단체"
            }
        ]
    }
